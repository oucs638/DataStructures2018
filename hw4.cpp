...
nodeptr mread(void) // the function to read and create a sparse matrix with linked list
{ // declare
    ...
    if (!numHeads) {
        headNode->right = headNode; // if the sparse matrix is empty just create a head
    } else {
        for (int i = 0; i < numHeads; ++i) { // initialize all head nodes
            temp = new Node;
            hdnode[i] = temp;
            hdnode[i]->down = temp;
            hdnode[i]->right = temp;
        ...
        // let the last node of this row point to the first node of this row
        last->right = hdnode[currentRow];
        // link all head nodes
...
{ // output the sparse matrix mainly row
    ...
    for (int i = 0; i < headNode->u.element.row; ++i) { // output
        for (temp = head->right; temp != head; temp = temp->right) {
            cout.setf(ios_base::left, ios_base::adjustfield);
            cout << setw(4) << temp->u.element.row
                 << setw(4) << temp->u.element.col
                 << setw(4) << temp->u.element.val << endl;
        }
        head = head->u.next;
    }
}
nodeptr mtranspose(nodeptr req)
{
    ...
    nodeptr res, temp, last, connect, reqtmp, reqhd = req->down;
    // set the number of rows and columns
    int numRows = req->u.element.col, numCols = req->u.element.row;
    int row = 0, col = 0, val = 0, currentRow = 0;
    // create the headNode of linked list
    ...
    // create the linked list
    if (!numHeads) {
        res->right = res; // if the sparse matrix is empty just create a head
    } else {
        for (int i = 0; i < numHeads; ++i) { // initialize all head nodes
            temp = new Node;
            hdnode[i] = temp;
...
nodeptr mmult(nodeptr req1, nodeptr req2)
{
    ...
    nodeptr res, temp, last, connect, tmp1, tmp2, head1 = req1->right, head2;
    int numRows = req1->u.element.row, numCols = req2->u.element.col;
    int row = 0, col = 0, val = 0, currentRow = 0;
    // create the headNode of linked list
    int numHeads = (numCols > numRows) ? numCols : numRows;
    res = new Node;
    ...
    if (!numHeads) {
        ...
        for (int i = 0; i < numRows; ++i) {
            for (int j = 0; j < numCols; ++j) {
                if (sum[i][j] != 0) {
                    ...
                    last->right = temp; // link row list
                    last = temp;
                    connect = hdnode[col]->down; // link column list
                    if (connect == hdnode[col]) {
                        hdnode[col]->down = temp;
                        temp->down = hdnode[col];
                    } else if (connect != hdnode[col]) {
        ...
        last->right = hdnode[currentRow]; // let the last node of this row point to the first node
        for (int i = 0; i < numHeads - 1; ++i) { // link all head nodes
            hdnode[i]->u.next = hdnode[i + 1];
        }
        hdnode[numHeads - 1]->u.next = res;
        res->down = hdnode[0];
